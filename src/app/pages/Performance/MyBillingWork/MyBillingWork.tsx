...
import { performanceUserColumns } from 'app/Components/CustomTable/CustomColoums/PerformanceUserColoums'
...
const BillingWorkUser = () => {
  const workBillingUser = useSelector((State:IRootState)=>State.PerformanceStateData.userList)
  ...
  const [filterAttr, setFilterAttr] = useState(initialState)
    ...
    const rows = useMemo(() => {
        const newList = workBillingUser?.map((item: any) => {
            return {
            ...item,
        }
        })
  ...
  const columns = React.useMemo(() => performanceUserColumns, [])
...
export default BillingWorkUser